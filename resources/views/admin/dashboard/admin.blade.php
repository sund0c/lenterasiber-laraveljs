...
@section('content')
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.5rem">
        ...
        @foreach ([['label' => 'Kabar Lentera', 'icon' => '📰', 'data' => $stats['kabar'], 'color' => '#3b82f6', 'key' => 'KABAR'], ['label' => 'Podcast', 'icon' => '🎙', 'data' => $stats['podcast'], 'color' => '#8b5cf6', 'key' => 'PODCAST'], ['label' => 'Komik', 'icon' => '📖', 'data' => $stats['komik'], 'color' => '#10b981', 'key' => 'KOMIK']] as $item)
            <div class="card" style="border-top:3px solid {{ $item['color'] }}">
                <div class="card-body" style="padding:1.2rem">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
                        <span style="font-size:1.4rem">{{ $item['icon'] }}</span>
                        <span
                            style="font-size:0.75rem;font-weight:600;color:{{ $item['color'] }};background:{{ $item['color'] }}18;padding:3px 10px;border-radius:20px">
                            {{ $item['label'] }}
                        </span>
                    </div>
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.5rem;text-align:center">
                        <div>
                            <div style="font-size:1.6rem;font-weight:700;color:var(--text)">{{ $item['data']['total'] }}
                            </div>
                            <div style="font-size:0.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">
                                Total</div>
                        </div>
                        <div>
                            <div style="font-size:1.6rem;font-weight:700;color:#10b981">{{ $item['data']['published'] }}
                            </div>
                            <div style="font-size:0.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">
                                Publish</div>
                        </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:1rem;margin-bottom:1.5rem">
        ...
        <div class="card">
            ...
            <div class="card-body" style="padding:1rem;display:grid;grid-template-columns:1fr 1fr;gap:8px">
                <a href="{{ route('admin.konten.create', 'KABAR') }}" class="btn-secondary"
                    style="text-align:center;font-size:0.78rem">+ Artikel</a>
                <a href="{{ route('admin.konten.create', 'PODCAST') }}" class="btn-secondary"
                    style="text-align:center;font-size:0.78rem">+ Podcast</a>
                <a href="{{ route('admin.konten.create', 'KOMIK') }}" class="btn-secondary"
                    style="text-align:center;font-size:0.78rem">+ Komik</a>
                <a href="{{ route('admin.users.create') }}" class="btn-secondary"
                    style="text-align:center;font-size:0.78rem">+ Staf</a>
            </div>
        </div>
                ...
                <tbody>
                    @forelse($recentAudit as $log)
                        <tr>
                            ...
                            <td>
                                @php
                                    $c = str_contains($log->action, 'delete')
                                        ? '#ef4444'
                                ...
                                @endphp
                                <code
                                    style="font-size:0.72rem;background:{{ $c }}18;color:{{ $c }};padding:2px 6px;border-radius:4px">{{ $log->action }}</code>
                            </td>
                            <td style="font-size:0.75rem;color:var(--muted)">
                                {{ $log->entity_type ? $log->entity_type . '#' . $log->entity_id : '—' }}</td>
                            <td style="font-size:0.75rem;color:var(--muted);font-family:monospace">
                                {{ $log->ip_address ?? '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" style="text-align:center;padding:1.5rem;color:var(--muted)">Belum ada
                                aktivitas.</td>
                        </tr>
                    @endforelse
                </tbody>
