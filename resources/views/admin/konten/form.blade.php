...
@section('content')
    ...
    <div style="max-width:760px">
        <form method="POST"
            action="{{ $item ? route('admin.konten.update', [$label, $item->id]) : route('admin.konten.store', $label) }}"
            ...
            <div class="card" style="margin-bottom:1rem">
                <div class="card-header"><strong>Informasi
                        {{ match ($label) {'KABAR' => 'Kabar Lentera','KOMIK' => 'Komik Lentera','PODCAST' => 'Podcast Lentera',default => ''} }}</strong>
                </div>
                <div class="card-body">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="episode_number"
                                    ...
                                    placeholder="{{ $label === 'KOMIK' ? 'Episode 1' : 'EP.08' }}" maxlength="20" required>
                    ...
                    {{-- Judul --}}
                    <div class="form-group">
                        <label>{{ match ($label) {'KABAR' => 'Judul Artikel','KOMIK' => 'Judul Komik','PODCAST' => 'Judul Episode',default => 'Judul'} }}
                    ...
                    @if ($isAdmin)
                        <div class="form-group">
                            ...
                            <input type="text" name="category" class="form-input @error('category') is-error @enderror"
                                ...
                                maxlength="100" @if ($label === 'KABAR') list="category-suggestions" @endif>
                            @if ($label === 'KABAR')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="excerpt" rows="2" class="form-input @error('excerpt') is-error @enderror"
                            placeholder="{{ match ($label) {
                    ...
                    <div class="form-group">
                        <label>{{ match ($label) {'KABAR' => 'Isi Artikel Lengkap','KOMIK' => 'Penjelasan Lengkap','PODCAST' => 'Catatan / Transkrip Episode',default => 'Konten'} }}
                            ...
                            @else
                                <span style="color:var(--muted);font-weight:normal">(opsional)</span>
                            @endif
                        </label>
                        <textarea name="content" rows="18" class="form-input @error('content') is-error @enderror"
                            placeholder="Tulis konten lengkap di sini..." @if ($label === 'KABAR') required @endif>{{ old('content', $item->content ?? '') }}</textarea>
