...
@section('content')
    ...
    <div style="margin-bottom:1rem;display:flex;gap:8px;align-items:center">
        <form method="GET" action="{{ route('admin.kabar.index') }}" style="display:flex;gap:8px;flex:1;max-width:460px">
            ...
            @if ($search)
                <a href="{{ route('admin.kabar.index', ['sort' => $sort, 'dir' => $dir]) }}" class="btn-secondary"
    ...
    <div class="card">
        @if ($items->isEmpty())
            <div class="card-body" style="text-align:center;padding:3rem;color:var(--muted)">
                @if ($search)
                    Tidak ada kabar yang cocok dengan <em>"{{ $search }}"</em>.
                    <a href="{{ route('admin.kabar.index') }}">Tampilkan semua</a>
                @else
                        ...
                        @foreach ($items as $item)
                            ...
                            <tr>
                                <td>
                                    ...
                                    @else
                                        <div
                                            style="width:56px;height:40px;background:var(--bg);border-radius:6px;border:1px solid var(--border)">
                                        </div>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    @if ($item->category)
                                        <span
                                            style="font-size:0.72rem;font-weight:600;padding:2px 8px;border-radius:4px;background:#3b82f620;color:#3b82f6">
                                            {{ $item->category }}
                                        </span>
                                    @else
                                ...
                                <td>
                                    <div style="display:flex;gap:4px;flex-wrap:wrap">
                                        @if ($locked)
                                            <a href="{{ route('admin.kabar.show', $item->id) }}" class="btn-secondary"
                                                style="padding:4px 8px;font-size:0.72rem">Lihat</a>
                                        @else
                                            <a href="{{ route('admin.kabar.edit', $item->id) }}" class="btn-secondary"
                                                style="padding:4px 8px;font-size:0.72rem">Edit</a>
                                            @if ($isAdmin)
                                                @if ($item->status === 'draft')
                                                    <form method="POST"
                                                        action="{{ route('admin.kabar.publish', $item->id) }}"
                                                        style="display:inline">
                                                        @csrf
                                                        <button type="submit" class="btn-primary"
                                                            style="padding:4px 8px;font-size:0.72rem;width:auto">Publish</button>
                                                    </form>
                                                @else
                                                    <form method="POST"
                                                        action="{{ route('admin.kabar.unpublish', $item->id) }}"
                                                        style="display:inline">
                                                        @csrf
                                                        <button type="submit" class="btn-secondary"
                                                            style="padding:4px 8px;font-size:0.72rem">Unpublish</button>
                                                    </form>
                                                @endif
                                            @endif
